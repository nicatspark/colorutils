...
const autoCssVarOnEvent =
  ...
  ({ el, cssVar }: CssVarToObject) => {
    ...
    const newClrObj = lighten(light)(rootClr)
...
const webHslToObject = (webHsl: HSL) => {
  ...
  const clr = (data[1] as string)
    .replace(/[^0-9]+/g, ',')
    .split(',')
    .filter(Boolean)
  if (!clr[0] || !clr[1] || !clr[2]) throw new Error('An error occurred')
  return hslToObject({
    hue: +clr[0],
    saturation: +clr[1],
    lightness: +clr[2],
    alpha: Number(clr[3]) || 1,
  })
}
